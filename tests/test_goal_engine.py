...
@pytest.fixture
def ge(tmp_path):
    """GoalEngine с изолированной SQLite."""
    return GoalEngine(sqlite_path=str(tmp_path / "goal_test.db"))


def test_create_goal(ge):
    goal = ge.create_goal(
        title="Test Goal",
...
def test_plan_goal_nonexistent(ge):
    ge.plan_goal("nonexistent", ["step1"])  # не должен падать


def test_start_execution(ge):
...
def test_start_execution_over_budget(ge):
...
def test_get_next_goal_priority_order(ge):
...
def test_get_next_goal_roi_tiebreaker(ge):
...
def test_get_next_goal_none_when_empty(ge):
    assert ge.get_next_goal() is None


def test_get_next_goal_skips_non_pending(ge):
...
def test_escalation_after_30_min(ge):
...
def test_get_all_goals_filter_status(ge):
