...
class DummyLLM:
    async def call_llm(self, **kwargs):
        return '{"summary":"stuck workflow detected","recommended_actions":["reset_or_fail_stuck_goal"],"risk_level":"high"}'
...
def test_autonomy_overseer_async_diagnostics(tmp_path):
    db = tmp_path / 'auto.db'
    store = AutonomyProposalStore(sqlite_path=str(db))
    overseer = AutonomyOverseer(stuck_tick_threshold=1, llm_router=DummyLLM())
    report = asyncio.run(overseer.inspect_async(
        tick_count=5,
        proposal_store=store,
        workflow_sessions=[{'goal_id': 'g1', 'state': 'running', 'last_tick': 1}],
    ))
    assert report['finding_count'] == 1
    assert report['diagnostics']['risk_level'] == 'high'
