#!/usr/bin/env python3
"""Ko-fi browser auth helper (storage_state capture)."""
...
async def browser_capture(timeout_sec: int, storage_path: str, headless: bool, auto_submit: bool) -> int:
    email = str(os.getenv("KOFI_EMAIL", "") or os.getenv("EMAIL", ""))
    password = str(os.getenv("KOFI_PASSWORD", "") or os.getenv("PASSWORD", ""))
    out = Path(storage_path)
    if not out.is_absolute():
        out = PROJECT_ROOT / out
    out.parent.mkdir(parents=True, exist_ok=True)

    print("\n=== Ko-fi Browser Session Capture ===")
    print("1) Откроется Ko-fi login")
    print("2) Заверши вход вручную (captcha/2FA если нужно)")
    print("3) Скрипт дождется /manage и сохранит storage_state")
    print(f"Output storage: {out}")
    if headless:
        print("WARNING: headless=True может ухудшать успешность логина.")

    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=headless, args=_chromium_launch_args())
        context = await browser.new_context(viewport={"width": 1366, "height": 900})
        page = await context.new_page()
        await page.goto("https://ko-fi.com/login", wait_until="domcontentloaded", timeout=120000)
        ...
        if password:
            for sel in ("input[type='password']", "input[name='password']"):
                try:
                    ...
                    continue
        if auto_submit:
            for sel in ("button[type='submit']", "button:has-text('Sign in')", "button:has-text('Log in')", "button:has-text('Войти')"):
                try:
        ...
        print("Waiting for successful login markers (/manage or /manage/shop)...")
        ok = False
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Ko-fi auth helper")
    sub = ap.add_subparsers(dest="cmd", required=True)

    p_capture = sub.add_parser("browser-capture", help="Capture logged-in Ko-fi browser session")
    p_capture.add_argument("--timeout-sec", type=int, default=420)
    p_capture.add_argument("--storage-path", default="runtime/kofi_storage_state.json")
