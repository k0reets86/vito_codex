...
def generate_fresh_assets(
    ...
    *,
    ...
    write_minimal_pdf(
        "\n".join(
            [
                f"{topic[:80]} {ts}",
                topic[:120],
                "Fresh artifact bundle generated by VITO",
                f"Run tag: {ts}",
    ...
    write_placeholder_png(str(cover_path), 1280, 720, text=(topic or "VITO")[:24])
    write_placeholder_png(str(thumb_path), 600, 600, text=(topic or "VITO")[:18])
    write_placeholder_png(str(image_path), 1200, 630, text=(topic or "VITO")[:22])
...
def build_platform_bundle(platform: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    assets = (
        generate_fresh_assets(
            topic or title or "VITO Fresh Asset",
