# VITO Dashboard Source Snapshot

## Source: /home/vito/vito_dashboard_src/src/components/Dashboard.tsx
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Initialize RSS sources from config if not already set
  ...
  const renderContent = () => {
    ...
    switch (currentSection.id) {
        ...
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {Object.entries(data.status).map(([key, value]) => (
              <div key={key} className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-3xl border border-slate-700 shadow-xl hover:border-emerald-500/30 transition-all group">
                <div className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-3 group-hover:text-emerald-400 transition-colors">{key}</div>
                <div className="text-xl font-mono text-white truncate" title={value}>{value}</div>
              </div>
            ))}
          </div>
        );
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-900/50 border-b border-slate-700">
                    <th className="p-5 text-slate-400 font-bold uppercase text-xs tracking-wider">Модуль</th>
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {data.agents.map((agent, i) => (
                    <tr key={i} className="hover:bg-slate-700/30 transition-colors">
                      <td className="p-5 font-mono text-sm text-emerald-400">{agent.module}</td>
                      <td className="p-5 text-sm text-slate-300">{agent.lastModified}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        return (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {data.platforms.map((p, i) => (
              <div key={i} className="bg-slate-800/50 backdrop-blur-sm p-5 rounded-2xl border border-slate-700 flex flex-col items-center justify-center text-center hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all cursor-default group">
                <div className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center mb-3 group-hover:bg-indigo-500/20 transition-colors">
                  <Layers size={18} className="text-slate-400 group-hover:text-indigo-400" />
                </div>
        ...
        return (
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-3xl border border-slate-700 overflow-hidden shadow-2xl">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900/50 border-b border-slate-700">
                  <th className="p-5 text-slate-400 font-bold uppercase text-xs tracking-wider">Дата</th>
                  <th className="p-5 text-slate-400 font-bold uppercase text-xs tracking-wider">Расход</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-700/50">
                {data.finance.map((f, i) => (
                  <tr key={i} className="hover:bg-slate-700/30 transition-colors">
                    <td className="p-5 text-sm text-slate-300">{f.date}</td>
                    <td className="p-5 font-mono text-base text-rose-400 font-bold">${parseFloat(f.spend).toFixed(4)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      case 'events':
        return (
          <div className="bg-slate-900/80 backdrop-blur-md p-8 rounded-3xl border border-slate-700 shadow-2xl overflow-hidden">
              ...
              {data.events.map((ev, i) => {
                ...
                return (
                  <div key={i} className={`p-5 rounded-2xl border ${
                    isError ? 'bg-rose-950/20 border-rose-900/50 text-rose-200' : 
                    isWarning ? 'bg-amber-950/20 border-amber-900/50 text-amber-200' :
                    'bg-slate-800/50 border-slate-700 text-slate-300'
                  } font-mono text-xs whitespace-pre-wrap break-words shadow-sm`}>
                    <div className="flex items-start gap-3">
                      {isError ? <AlertTriangle size={14} className="mt-0.5 shrink-0" /> : null}
                      <span>{ev}</span>
                    </div>
                  </div>
                );
          ...
          <div className="space-y-8">
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-3xl border border-slate-700 overflow-hidden shadow-2xl">
              <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-900/30">
                ...
                <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Write-Only Secrets</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <tbody className="divide-y divide-slate-700/50">
      ...
      <header className="fixed top-0 left-0 right-0 h-20 bg-slate-900/60 backdrop-blur-xl border-b border-slate-800 z-50 flex items-center justify-between px-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Control Center v2.0</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex flex-col items-end">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Система</span>
