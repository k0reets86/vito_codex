...
class ConversationEngine:
    ...
    async def _deterministic_owner_route(self, text: str) -> dict[str, Any] | None:
        ...
        if (
            self._has_keywords(normalized, ("printful", "принтфул"), fuzzy=True)
            and self._has_keywords(normalized, ("etsy", "этси", "етси"), fuzzy=True)
    ...
    async def _dispatch_action(self, action: str, params: dict) -> str:
        ...
        if action == "run_printful_etsy_sync":
            ...
            create = await self.agent_registry.dispatch("listing_create", platform="printful", data=payload)
            if not create or not create.success:
                return f"Printful: не удалось создать товар ({getattr(create, 'error', 'unknown')})."
            snap = await self.agent_registry.dispatch("sales_check", platform="etsy")
